...
bosyuuing_guild_channel_dict = {}

@client.event
async def on_message(message):
    if message.content == "!start":
        if message.guild in bosyuuing_guild_channel_dict:
            await message.channel.send(f"現在<#{bosyuuing_guild_channel_dict[message.guild].id}>で募集されています。")
        else:
            bosyuuing_guild_channel_dict[message.guild] = message.channel
            await bosyuu(message,bosyuuing_guild_channel_dict)

async def bosyuu(message,bosyuuing_guild_channel_dict):
    sankasya_list = []
    ...
    del bosyuuing_guild_channel_dict[message.guild]
